defmodule RyugraphEx.GraphTest do
  ...
  describe "create_node/3" do
    test "creates a node with properties", %{conn: conn} do
      assert {:ok, node} = Graph.create_node(conn, "Person",
        id: 1,
        name: "Alice",
        age: 30,
    ...
    test "creates nodes with different labels", %{conn: conn} do
      assert {:ok, person} = Graph.create_node(conn, "Person",
        id: 4,
        name: "Diana"
      )

      assert {:ok, product} = Graph.create_node(conn, "Product",
        id: 1,
        name: "Laptop",
        price: 999.99
      )
    ...
    test "handles various property types", %{conn: conn} do
      assert {:ok, node} = Graph.create_node(conn, "Product",
        id: 2,
        name: "Phone",
        price: 599.99,
    ...
    test "handles constraint violations", %{conn: conn} do
      ...
      assert {:error, _reason} = Graph.create_node(conn, "Person",
  ...
  describe "create_relationship/5" do
    ...
    test "creates relationship between nodes", %{conn: conn, alice: alice, bob: bob} do
      assert {:ok, rel} = Graph.create_relationship(conn,
    ...
    test "creates relationships with different types", %{conn: conn, alice: alice, product: product} do
      assert {:ok, rel} = Graph.create_relationship(conn,
        alice.id, product.id, "OWNS",
        quantity: 1,
  ...
  describe "find_nodes/4" do
    ...
    test "supports ORDER BY option", %{conn: conn} do
      assert {:ok, nodes} = Graph.find_nodes(conn, "Person", nil,
        order_by: "age",
        limit: 10
      )
    ...
    test "combines WHERE, ORDER BY, and LIMIT", %{conn: conn} do
      ...
      ages = Enum.map(nodes, & &1[:properties][:age])
      assert ages == [40, 35]  # Eve and Charlie
    end
  ...
  describe "shortest_path/4" do
    ...
    test "finds direct path", %{conn: conn, people: people} do
      assert {:ok, path} = Graph.shortest_path(conn,
        people[1].id,
        people[2].id
      )
      ...
      assert length(path[:rels]) == 1  # Direct connection
    end
    ...
    test "finds multi-hop path", %{conn: conn, people: people} do
      ...
      assert length(path[:rels]) >= 2  # At least 2 hops needed
    end
    ...
    test "respects relationship type filter", %{conn: conn, people: people} do
      ...
      assert {:ok, path1} = Graph.shortest_path(conn,
        people[1].id,
        people[6].id
      )

      # With filter - should only use KNOWS relationships
      assert {:ok, path2} = Graph.shortest_path(conn,
        people[1].id,
        people[6].id,
        rel_label: "KNOWS"
      )
    ...
    test "respects max_length constraint", %{conn: conn, people: people} do
      ...
      assert {:error, "No path found"} = Graph.shortest_path(conn,
        people[1].id,
        people[6].id,
        max_length: 1
      )

      assert {:ok, _path} = Graph.shortest_path(conn,
        people[1].id,
        people[6].id,
        max_length: 3
      )
    ...
    test "returns error when no path exists", %{conn: conn, people: people} do
      ...
      {:ok, isolated} = Graph.create_node(conn, "Person",
        id: 99,
        name: "Isolated"
      )
    ...
    test "handles same source and destination", %{conn: conn, people: people} do
      ...
      result = Graph.shortest_path(conn,
        people[1].id,
        people[1].id
      )
  ...
  describe "get_neighbors/3" do
    setup %{conn: conn} do
      ...
      {:ok, center} = Graph.create_node(conn, "Person",
        id: 1,
        name: "Center"
      )
    ...
    test "filters by relationship label", %{conn: conn, center: center, neighbors: neighbors} do
      ...
      assert {:ok, knows_neighbors} = Graph.get_neighbors(conn, center.id,
        rel_label: "KNOWS"
      )

      assert {:ok, works_neighbors} = Graph.get_neighbors(conn, center.id,
        rel_label: "WORKS_WITH"
      )
    ...
    test "handles depth parameter for multi-hop neighbors", %{conn: conn, center: center} do
      ...
      {:ok, [first_neighbor | _]} = Graph.get_neighbors(conn, center.id,
        direction: :out
      )
      ...
      assert {:ok, depth1} = Graph.get_neighbors(conn, center.id,
        depth: 1
      )

      # Depth 2 should include second-degree
      assert {:ok, depth2} = Graph.get_neighbors(conn, center.id,
        depth: 2
      )
  ...
  describe "update_node/3" do
    ...
    test "updates existing properties", %{conn: conn, node: node} do
      ...
      assert updated[:properties][:name] == "Original"  # Unchanged
    end
    ...
    test "adds new properties", %{conn: conn, node: node} do
      ...
      assert updated[:properties][:name] == "Original"  # Preserved
    end
    ...
    test "handles multiple property types", %{conn: conn, node: node} do
      assert {:ok, _updated} = Graph.update_node(conn, node.id,
        age: 27,  # integer
        city: "Portland",  # string
        active: true  # boolean (if supported)
      )
    end
  ...
  describe "delete_node/3" do
    ...
    test "fails to delete node with relationships without detach", %{conn: conn, node: node} do
      ...
      assert {:error, _reason} = Graph.delete_node(conn, node.id,
        detach: false
      )
  ...
  describe "complex graph operations" do
    test "builds and queries a social network", %{conn: conn} do
      # Create people
      people = for i <- 1..5 do
        {:ok, person} = Graph.create_node(conn, "Person",
          id: i,
          name: "Person#{i}",
          age: 20 + i * 5
        )
      ...
      assert {:ok, friends} = Graph.get_neighbors(conn,
        people[1].id,
        direction: :out,
        rel_label: "KNOWS"
      )
    ...
    test "product ownership graph", %{conn: conn} do
      ...
      assert {:ok, alice_products} = Graph.get_neighbors(conn,
        alice.id,
        direction: :out,
        rel_label: "OWNS"
      )
      ...
      # Find Bob's products
      assert {:ok, bob_products} = Graph.get_neighbors(conn,
        bob.id,
        direction: :out,
        rel_label: "OWNS"
      )
